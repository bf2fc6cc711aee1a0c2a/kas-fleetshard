...
public class KafkaCluster extends AbstractKafkaCluster {
    ...
    private void asReserveDeployments(ManagedKafka managedKafka) {
        ...
        createOrUpdatePausePodDeployment(managedKafka, kafka, ZOOKEEPER_SUFFIX, k -> k.getSpec().getZookeeper(),
                s -> s.getTemplate().getPod(), s -> s.getReplicas(), s -> s.getResources());
        createOrUpdatePausePodDeployment(managedKafka, kafka, EXPORTER_SUFFIX, k -> k.getSpec().getKafkaExporter(),
                s -> s.getTemplate().getPod(), s -> 1, s -> s.getResources());
        createOrUpdatePausePodDeployment(managedKafka, kafka, KAFKA_SUFFIX, k -> k.getSpec().getKafka(),
                s -> s.getTemplate().getPod(), s -> s.getReplicas(), s -> s.getResources());
        createOrUpdatePausePodDeployment(managedKafka, kafka, CRUISECONTROL_SUFFIX, k -> k.getSpec().getKafka(),
                s -> s.getTemplate().getPod(), s -> 1, s -> s.getResources());
    }

    private <T> void createOrUpdatePausePodDeployment(ManagedKafka managedKafka, Kafka kafka, String nameSuffix,
        ...
        Deployment pause = PausePodConverter.asPauseDeployment(current, managedKafka, name, kafka.getMetadata(),
                replicasExtractor.apply(spec), template,
                resourceExtractor.apply(spec));

        if (!Objects.equals(current, pause)) {
            OperandUtils.createOrUpdate(kubernetesClient.apps().deployments(), pause);
        }
    }
