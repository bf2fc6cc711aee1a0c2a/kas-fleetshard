 ...
 * control plane.
 *
 * Actual modifications to the local ManagedKafka are added to a simple work
 * queue. For now that is just the common pool, but you could envision using a
...
public class ManagedKafkaSync {
    ...
    public void syncKafkaClusters(List<ManagedKafka> remoteManagedKafkas, Executor executor) {
        ...
        MixedOperation<ManagedKafka, ManagedKafkaList, Resource<ManagedKafka>> managedKafkaResources = client
                .customResources(crdContext, ManagedKafka.class, ManagedKafkaList.class);
        ...
        for (ManagedKafka remoteManagedKafka : remoteManagedKafkas) {
            ...
            // there are two strategies - use the informer or we can do direct lookups when
            // needed.
            // ManagedKafka existing =
            // managedKafkaResources.inNamespace(namespace).withName(remoteManagedKafka.getMetadata().getName()).get();

            // using the informer seems to be the right approach as it should generally be
            // up-to-date
