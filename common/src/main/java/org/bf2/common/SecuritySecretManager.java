...
public class SecuritySecretManager {
    ...
    private static void kafkaTlsKeyPairUpdate(ManagedKafka managedKafka) {
        SecretKeySelector certRef = new SecretKeySelector();
        certRef.setName("tls.crt");
        certRef.setKey(managedKafka.getSpec().getEndpoint().getTls().getCert());

        SecretKeySelector keyRef = new SecretKeySelector();
        keyRef.setName("tls.key");
        keyRef.setKey(managedKafka.getSpec().getEndpoint().getTls().getKey());

        //TO-DO : Clear the String value from ManagedKafka
    }
